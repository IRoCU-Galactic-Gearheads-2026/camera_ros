...
private:
  ...
  void requestComplete(libcamera::Request *const request);
  void process(libcamera::Request *const request);
  void postParameterChange(const std::vector<rclcpp::Parameter> &parameters);
...
libcamera::Size get_sensor_format(const std::string &format_str)
{
  ...
  if (std::regex_match(format_str, match, pattern)) {
    try {
      return libcamera::Size {static_cast<unsigned int>(std::stoi(match[1].str())), static_cast<unsigned int>(std::stoi(match[2].str()))};
    } catch (...) {}
  }
  throw std::runtime_error("Invalid sensor_mode. Expected [width]:[height] but got " + format_str);
}

void compressImageMsg(const sensor_msgs::msg::Image &source,
                 ...
                 const std::vector<int> &params = std::vector<int>())
{
  ...
  if (cv_ptr->encoding == enc::BGR8 || cv_ptr->encoding == enc::BGRA8 ||
    ...
    if (enc::hasAlpha(cv_ptr->encoding)) temp = cv_bridge::cvtColor(tempThis, enc::BGRA8);
...
CameraNode::CameraNode(const rclcpp::NodeOptions &options)
    : Node("camera", options),
      ...
      cim(this->get_node_base_interface(), this->get_node_services_interface(), this->get_node_logging_interface()
#if CIM_HAS_QoS
      ...
      )
{
  ...
  const libcamera::StreamRole role = get_role(declare_parameter<std::string>("role", "viewfinder", param_descr_role));
  ...
  orientation_range.from_value = 0; orientation_range.to_value = 270; orientation_range.step = 90;
  param_descr_orientation.integer_range.push_back(orientation_range);
  param_descr_orientation.read_only = true;
  const int angle = declare_parameter<int>("orientation", 0, param_descr_orientation);
  ...
  const rclcpp::ParameterValue &camera_id = declare_parameter("camera", rclcpp::ParameterValue {}, param_descr_ro.set__dynamic_typing(true));
  ...
  if (libcamera::PixelFormat::fromString(format) != libcamera::formats::MJPEG) {
    ...
    jpeg_range.from_value = 1; jpeg_range.to_value = 100; jpeg_range.step = 1;
  ...
  pub_image_compressed = this->create_publisher<sensor_msgs::msg::CompressedImage>("~/image_raw/compressed", 1);
  pub_ci = this->create_publisher<sensor_msgs::msg::CameraInfo>("~/camera_info", 1);

  const int ec_start = camera_manager.start();
  if (ec_start < 0) throw std::runtime_error("failed to start camera manager");
  if (camera_manager.cameras().empty()) throw std::runtime_error("no cameras available");
  ...
  case rclcpp::ParameterType::PARAMETER_INTEGER:
  {
    ...
    if (id >= camera_manager.cameras().size()) throw std::runtime_error("camera with id does not exist");
  ...
  case rclcpp::ParameterType::PARAMETER_STRING:
  {
    ...
    if (!camera) throw std::runtime_error("camera with name does not exist");
  } break;
  default:
    throw std::runtime_error("unsupported camera parameter type");
  }

  if (!camera) throw std::runtime_error("failed to find camera");
  if (camera->acquire()) throw std::runtime_error("failed to acquire camera");

  // REVERTED TO SINGLE STREAM CONFIG TO AVOID ISP PI 5 CRASH
  ...
  std::unique_ptr<libcamera::CameraConfiguration> cfg = camera->generateConfiguration(roles);
  ...
  if (common_fmt.empty()) throw std::runtime_error("camera does not provide any of the supported pixel formats");
  ...
  if (format.empty()) {
    ...
    if (!format_requested.isValid() || std::find(common_fmt.begin(), common_fmt.end(), format_requested) == common_fmt.end()) {
      throw std::runtime_error("unsupported pixel format");
    }
    scfg.pixelFormat = format_requested;
  }

  if (!size.isNull()) scfg.size = size;
  ...
  if (camera->configure(cfg.get()) < 0) throw std::runtime_error("failed to configure camera");

  std::string cname = camera->id() + '_' + scfg.size.toString();
  cname.erase(std::remove_if(cname.begin(), cname.end(), [](const char &x) { return std::isspace(x); }), cname.cend());
  std::replace_if(cname.begin(), cname.end(), [](const char &x) { return !std::isalnum(x); }, '_');

  if (!cim.setCameraName(cname)) throw std::runtime_error("camera name must only contain alphanumeric characters");

  const std::string &camera_info_url = declare_parameter<std::string>("camera_info_url", {}, param_descr_camera_info_url);
  if (!cim.loadCameraInfo(camera_info_url)) {
    if (!camera_info_url.empty()) cim.loadCameraInfo({});
  }
  ...
  if (camera->start(&parameter_handler.get_control_values())) throw std::runtime_error("failed to start camera");

  for (std::unique_ptr<libcamera::Request> &request : requests) {
    if (const int ret = camera->queueRequest(request.get()); ret < 0) {
      RCLCPP_WARN_STREAM(get_logger(), "failed to queue request");
    }
  }
}
...
CameraNode::~CameraNode()
{
  ...
  for (auto &[req, condvar] : request_condvars) condvar.notify_all();
  for (std::thread &thread : request_threads) thread.join();
  ...
  camera.reset();
  camera_manager.stop();
  for (const auto &e : buffer_info) munmap(e.second.data, e.second.size);
}
...
void CameraNode::process(libcamera::Request *const request)
{
  while (true) {
    ...
    if (request->status() == libcamera::Request::RequestComplete) {
      ...
      size_t bytesused = 0;
      for (const libcamera::FrameMetadata::Plane &plane : buffer->metadata().planes())
      ...
      RCLCPP_ERROR_STREAM(get_logger(), "request cancelled");
    }
...
void CameraNode::postParameterChange(const std::vector<rclcpp::Parameter> &parameters) {
...
#ifndef RCLCPP_HAS_PARAM_EXT_CB
rcl_interfaces::msg::SetParametersResult CameraNode::onParameterChange(const std::vector<rclcpp::Parameter> &parameters) {
